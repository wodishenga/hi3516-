...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    if (WDR_MODE_2To1_LINE == pstSnsState->enWDRMode)
    {
        ...
        else/* long exposure */
        {
            ...
            if (25 == gu32STimeFps)
            {
                if ((2700 <= u32LongIntTime) && (2700 > u32ShortIntTime) && (10520 > (u32LongIntTime + 2704)) && (40 <= gu32LGain))
                {
                    ...
                    pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = IMX335_LOW_8BITS(SGainDb);
                    pstSnsState->astRegsInfo[0].astI2cData[15].u32Data = IMX335_MID_8BITS(SGainDb);
                }
                else
                {
                    pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = IMX335_LOW_8BITS(gu32LGain);
                    pstSnsState->astRegsInfo[0].astI2cData[15].u32Data = IMX335_MID_8BITS(gu32LGain);
                }
            }
            //fps = 30, stime = 8.4ms
    ...
    else  //Linear
    {
        ...
        if (2160 < u32IntTime && 2700 > u32IntTime && 70 < gu32LGain)
        {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[3].u32Data = IMX335_LOW_8BITS(SGainDb);
            pstSnsState->astRegsInfo[0].astI2cData[4].u32Data = IMX335_MID_8BITS(SGainDb);
...
static HI_VOID cmos_get_inttime_max(VI_PIPE ViPipe, HI_U16 u16ManRatioEnable, HI_U32 *au32Ratio, HI_U32 *au32IntTimeMax, HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime)
{
    ...
    if ((WDR_MODE_2To1_LINE == pstSnsState->enWDRMode))
    {
        ...
        */
        #if 0
        u32BRLCalTemp = g_astimx335State[ViPipe].u32BRL * 2;
        g_astimx335State[ViPipe].u32RHS1_MAX = (((pstSnsState->au32FL[0] - u32BRLCalTemp) / 4 - 2) * 2 + 3) * 2;

        if (g_astimx335State[ViPipe].u32RHS1_MAX > u32BRLCalTemp)   //RHS1_MAX need less than BRL*2
        {
            g_astimx335State[ViPipe].u32RHS1_MAX = u32BRLCalTemp;
        }
        #endif
...
static HI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, HI_U8 u8Mode)
{
    ...
    switch (u8Mode)
    {
        ...
        case WDR_MODE_2To1_LINE:
            ...
            //pstSnsState->u8ImgMode = IMX335_5M_30FPS_10BIT_WDR_MODE;
            g_astimx335State[ViPipe].u32BRL = 1984 * 2;

            //pstSnsState->u32FLStd = IMX335_VMAX_4M_25FPS_10BIT_WDR;
            //pstSnsState->u8ImgMode = IMX335_4M_25FPS_10BIT_WDR_MODE;
