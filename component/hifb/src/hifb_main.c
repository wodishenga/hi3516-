...
HI_S32 hifb_AllocCompressBuffer(HIFB_PAR_S *par)
{
    ...
    HI_U32 u32LayerIndex = par->u32LayerID;
    //HI_U32 u32Frame_Size0_Len = 16;
    //HI_U32 u32Frame_Size1_Len = 16;
...
HI_S32 __init hifb_init(void* pArgs)
{
    ...
    //s32Ret = himedia_register(&hifbdev);
    if(s32Ret)
    {
...
static inline int HifbInterruptRoute(HI_S32 irq, HI_VOID *dev_id)
{
    ...
    // HIFB_DRV_GraphicsGetInt
    s_stDrvOps.HIFB_DRV_GraphicsGetINT(&u32IntStatus);
    // HIFB_DRV_GraphicsClearIntStatus
    /*
    ...
    */
    //s_stDrvOps.HIFB_DRV_GraphicsClearINTStatus(u32IntStatus);

    HIFB_TRACE(HI_DBG_DEBUG, "intstatus : %d\n",u32IntStatus);

    // HIFB_DRV_GraphicsGetInterruptDev
...
HI_VOID hifb_release_info(HIFB_PAR_S *par)
{
    ...
    if ((!IS_CURSOR_LAYER(par->u32LayerID)) || (!IS_SOFT_CURSOR()))
    {
        ...
        //s_stDrvOps.HIFB_DRV_SetIntCallback(HIFB_INTTYPE_VO_DISP, HI_NULL, par->u32LayerID, HI_NULL);
...
static HI_S32 hifb_release(struct fb_info *info, HI_S32 user)
{
    ...
    if (atomic_dec_and_test(&par->ref_count))
    {
        ...
        {
            memset(hifb_get_screen_base(info), 0, hifb_get_smem_len(info));
        }
    ...
    switch (cmd)
    {
        ...
        case FBIOPUT_SCREENINFO_HIFB:
        {
            ...
            if (info->vtable.fb_set_par)
            {
                ...
                //hifb_putcmap(&info->vtable, &info->cmap);
            }
            break;
        }
        ...
        case FBIOPAN_DISPLAY_HIFB:
        {
            ...
            #if 0
            if (xoffset > info->oinfo.sarea.w - xres ||yoffset > info->oinfo.sarea.h - yres)
            {
                s32Ret = -EINVAL;
                break;
            }
...
/* There is currently no chip that needs to support this feature, so comment out first. */
        ...
        case FBIOGET_CURSOR_INFO:
        {
            ...
            return copy_to_user(argp, &stCursorInfo, sizeof(HIFB_CURSOR_S));
        }
...
static HI_S32 hifb_compat_ioctl(struct fb_info *info, unsigned cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case FBIOPUT_SCREEN_ORIGIN_HIFB:
        {
            ...
            /* Record the original position first */
            stOldPos.s32XPos = pstDisplayInfo->stPos.s32XPos;
            stOldPos.s32YPos = pstDisplayInfo->stPos.s32YPos;
        ...
        case FBIOFLIP_SURFACE:
        {
            ...
            differ = addr - hifb_get_smem_start(info);
            yoffset = differ/hifb_get_line_length(info);
            xoffset = ((differ%info->hifb_get_line_length(info)) << 3)/(hifb_get_bits_per_pixel(info));
        ...
        case FBIOPUT_DYNAMIC_RANGE_HIFB:
        {
            ...
            if (copy_from_user(&enDynamicRange, argp, sizeof(DYNAMIC_RANGE_E)))
            {
                return -EFAULT;
            }
            s32Ret = hifb_setDynamicRange(par,enDynamicRange);
        ...
        case FBIOGET_DYNAMIC_RANGE_HIFB:
        {
            if (copy_to_user(argp, &(pstDisplayInfo->enDynamicRange), sizeof(DYNAMIC_RANGE_E)))
            {
                return -EFAULT;
            }
            return HI_SUCCESS;
...
HI_U32  * pu32AR_size,HI_U32  * pu32GB_size,HI_VOID* screen_base,HI_U32 offset)
{
...
static HI_S32 hifb_refresh_1buf(HI_U32 u32LayerId, HIFB_BUFFER_S *pstCanvasBuf)
{
  ...
  //  struct fb_var_screeninfo * pvar           = &info->var;
   // struct fb_fix_screeninfo * pfix           = &info->fix;
    ...
    #if 0
    if (stOsdData.u64BufferPhyAddr != stDst.stCanvas.u64PhyAddr)
    {
        pstPar->bModifying = HI_TRUE;

        pstPar->u32ParamModifyMask |= HIFB_LAYER_PARAMODIFY_DISPLAYADDR;
        pstPar->stBufInfo.u32ScreenAddr = stDst.stCanvas.u64PhyAddr;

        pstPar->bModifying = HI_FALSE;
    }
    #else
    // The address in the displayed info is not empty and has changed.
...
#if 0
        pstPar->stRefreshInfo.u64ScreenAddr = pstPar->stRefreshInfo.stDispBufInfo.u64PhyAddr[0];
#else
...
{
    HIFB_REFRESH_WORKQUEUE_S * pstWork = (HIFB_REFRESH_WORKQUEUE_S*)container_of(work, HIFB_REFRESH_WORKQUEUE_S, FenceRefreshWork);
    HI_U32                     u32LayerId = pstWork->u32LayerId;
    HIFB_BUFFER_S *            pstCanvasBuf     = pstWork->pstCanvasBuf;

    hifb_refresh_2buf(u32LayerId, pstCanvasBuf);

    DRV_HIFB_FENCE_IncRefreshTime();
...
static HI_S32 hifb_refresh_2buf_fence(HI_U32 u32LayerId, HIFB_BUFFER_S *pstCanvasBuf)
{
    ...
    {
        HIFB_ERROR("DRV_HIFB_FENCE_Create failed %s:%d!\n", __FUNCTION__, __LINE__);
        osal_vfree(pstWork);
        pstWork = NULL;
        return HI_FAILURE;
    }
...
//    struct fb_var_screeninfo * pvar             = &info->var;
  //  struct fb_fix_screeninfo * pfix             = &info->fix;
    ...
    unsigned long              lockflag;
    HIFB_OSD_DATA_S*           pstOsdData;
    HI_U64                     u64OsdBufferPhyAddr;
    HI_U64                     u64WorkBufAddr;
    HI_U32                     u32BufSize       =  ((hifb_get_line_length(info) * hifb_get_yres(info))+HIFB_ALIGNMENT)&(~HIFB_ALIGNMENT);
    HI_U32                     u32BytesPerPixel = 2;
    TDE_EXPORT_FUNC_S *pfnTdeExportFunc = NULL;
...
static HI_S32 hifb_refresh_2buf_immediate_display(HI_U32 u32LayerId, HIFB_BUFFER_S *pstCanvasBuf)
{
   ...
   // struct fb_var_screeninfo * pvar            = &info->var;
    ...
    memset(&tmp, 0x0, sizeof(stBlitOpt));
    memset(&stBlitOpt, 0, sizeof(HIFB_BLIT_OPT_S));
    ...
    if (pstPar->stCompressInfo.bCompressOpen)
    {
            ...
            || (stBlitOpt.bBlock == HI_TRUE && stBlitOpt.bCallBack == HI_TRUE))
        {
            ...
            pstCompressInfo->u32Frame_Size0 =  u32AR_size;//u32AR_size;//0x60b5;
            pstCompressInfo->u32Frame_Size1 =  u32GB_size;//u32GB_size;//0x6079;
        }
...
static HI_S32 hifb_freeccanbuf(HIFB_PAR_S *par)
{
    HIFB_SURFACE_S *           pstCanvasSur   = &par->CanvasSur;
...
{
    HIFB_REFRESH_INFO_S *      pstRefreshInfo   = &pstPar->stRefreshInfo;
    HI_U32                     u32IndexForInt   = pstRefreshInfo->stDispBufInfo.u32IndexForInt;
   ...
   // pfix               = &info->fix;
    ...
    else
    {
        ...
        if (pstPar->stCompressInfo.bCompressOpen)
        {
            ...
            pstPar->stCompressInfo.u32Frame_Size0 =  u32AR_size;//u32AR_size;//0x60b5;
            pstPar->stCompressInfo.u32Frame_Size1 =  u32GB_size;//u32GB_size;//0x6079;
...
static HI_S32 hifb_cursor_show(HI_U32 u32LayerId)
{
 ...
 #if 0
    /*In compression mode, TDE callbacks are required to drive the compression process*/
    if (pstPar->stCompressInfo.bCompressOpen)
    {
        stBlitOpt.bCallBack = HI_TRUE;
        stBlitOpt.pParam = &pstPar->u32LayerID;
        pstPar->stCompressInfo.stUpdateRect = stDisplayBuf.UpdateRect;
    }
#endif
...
static HI_VOID hifb_cursor_calcdispinfo(HI_U32 u32CursorId, HIFB_PAR_S *pstPar, const HIFB_POINT_S* pstCurNewPos)
{
    struct fb_info *   info              = g_stLayer[u32CursorId].pstInfo;
...
static HI_S32 hifb_vo_callback(HI_VOID *pParaml, HI_VOID *pParamr)
{
    ...
    struct fb_info *           info                 = g_stLayer[*pu32LayerId].pstInfo;
    HIFB_PAR_S *               pstPar               = (HIFB_PAR_S *)(info->par);
//    struct fb_var_screeninfo * pvar                 = &info->var;
//    struct fb_fix_screeninfo * pfix                 = &info->fix;
    HIFB_DISPLAY_INFO_S *      pstDisplayInfo       = &pstPar->stDisplayInfo;
...
#ifdef CONFIG_HIFB_FENCE_SUPPORT
    if (copy_to_user(argp, &stCanvasBuf, sizeof(HIFB_BUFFER_S)))
    {
        HIFB_ERROR("copy_to_user failed %s:%d\n", __FUNCTION__, __LINE__);
        return -EFAULT;
    }
#endif
    return s32Ret;
...
{
    // Modify the layer's buf refresh mode
...
static int fb_alloc_cmap(struct fb_cmap_s *cmap, int len, int transp) //TODO
{
...
HI_S32 hifb_print_layer_proc(struct fb_info * info, struct seq_file *p, HI_VOID *v)
{
    ...
    const HI_CHAR*             pszBufMode[]          =
        {"triple", "double", "single", "triple( no frame discarded)", "unkown"};
...
HI_S32 hifb_read_proc(struct osal_proc_dir_entry *entry)
{
    struct fb_info* info = (struct fb_info *)(entry->private);
    HIFB_PAR_S *par = (HIFB_PAR_S *)info->par;
...
#define TMP_BUF_LEN 32
    ...
    //CMPI_PROC_ITEM_S *item = s->private;
    struct fb_info *info;
    HIFB_PAR_S *pstPar;
