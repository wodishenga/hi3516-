...
/*图片降噪以及提高黑度值*/
HI_S32 SAMPLE_Noise_Reduction(VI_PIPE SnapPipe)
{
   ...
   s32Ret = HI_MPI_ISP_GetIspSharpenAttr(SnapPipe, &stSharpenAttr);
	...
	s32Ret = HI_MPI_ISP_GetIspHlcAttr(SnapPipe, &stIspHlcAttr);
	if (HI_SUCCESS != s32Ret)
	{
			SAMPLE_PRT("HI_MPI_ISP_GetIspHlcAttr failed with %#x!\n", s32Ret);
			return s32Ret;
	}
	stIspHlcAttr.bEnable = HI_TRUE;
	stIspHlcAttr.u8LumaTarget = 10;
	stIspHlcAttr.u8LumaThr = 248;
	s32Ret = HI_MPI_ISP_SetIspHlcAttr(SnapPipe, &stIspHlcAttr);
	if (HI_SUCCESS != s32Ret)
	{
			SAMPLE_PRT("HI_MPI_ISP_SetIspHlcAttr failed with %#x!\n", s32Ret);
			return s32Ret;
	}*/
...
HI_VOID SAMPLE_Yuv420_to_Mat(Mat &dst, unsigned char* pYUV420, int width, int height)
{
	if (!pYUV420) {
		return;
	}
...
HI_S32 SAMPLE_Flame_Detection(Mat src){
...
HI_S32 SAMPLE_Picture_Processing(VIDEO_FRAME_S* pVBuf)
{
		HI_S32 s32Ret = HI_SUCCESS;
		unsigned int w, h;
		char* pVBufVirt_Y;
